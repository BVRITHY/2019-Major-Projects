...
%   No_of_Speakers=6;
  No_of_Utterances=200;
args=['-A -D -T 1 -C ']; 
config1='D:/HTK-3.4.1/htk/config';
  for i= 1: No_of_Utterances
    %  for j=1:No_of_Utterances
         mySourceFile = ['D:/speechdatanew/sixspeaker/' int2str(i) '.wav'];
         myTargetFile = ['D:/speechdatanew/sixspeaker/' int2str(i) '.mfc'];
         rv= system([get_htk_path 'HCopy ' args  config1 ' '  mySourceFile ' '  myTargetFile]);
%   %end
  end
   fid = fopen( 'sixspeaker.txt', 'wt' );
for t=1:200
    mySourceFile = ['D:\speechdatanew\sixspeaker\' int2str(t) '.mfc'];
     fprintf( fid, '%s\n',mySourceFile );
end
%     args=['-A -D -T 1 -C ']; 
%      config1='D:/HTK-3.4.1/htk/config';
...
%  
    ...
    A3='sixspeaker.txt';
          ...
          for i= 1: 200
                     fid = fopen( 'sixspeaker.txt', 'wt' );
                         targetpath=['D:\speechdatanew\sixspeaker\' int2str(i) '.mfc'];
                         fprintf( fid, '%s\n',targetpath);
